...
| 08 | [evals](recipes/ollama-08-evals/) | the offline eval loop: `Dataset`, evaluators, `run_experiment` + `run_experiment_async`, `render_experiment_report`, `compare_experiments`, `send_experiment`, the `bir experiments` / `experiment-show` / `experiment-report` CLI, `bir.testing.capture_traces` |
